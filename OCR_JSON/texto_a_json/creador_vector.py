...
class TextoJson:
    ...
    def convertir_txt_json(self, ruta_destino):
        ...
        for n, caso in enumerate(self.casos):
            self.caso = caso
            self._extraer_datos_antecedentes_personales()
            self._extraer_datos_antecedentes_familiares()
            self._extraer_datos_inmunohistoquuímica_tumoral()
            self._extraer_datos_clasificacion()
            self._extraer_datos_estadia_tumoral()
            self._crear_json(ruta_destino + f"\\{n+1}.json")
            self._limpiar_data()
        self._limpiar_instancia()

    def _limpiar_instancia(self):
        """
    ...
    def _extraer_datos_antecedentes_personales(self):
        """
        ...
        for linea in self.caso:
            ...
            if match:
                ...
                if edad_valor:
                    try:
                        unidades = self._extract_units(edad_valor)
                        edad_dict = {}
                        ...
                        if "negado" in linea:
                            ...
                            if len(linea.split("negado")[1].strip()) > 0:
                                ...
                                for observacion in observaciones:
                                    unidades = self._extract_units(observacion)
                                    if unidades:
                            ...
                            for parte in partes:
                                ...
                                for keyword in matches:
                                    if keyword in parte:
                                        ...
                                        # Usar _extract_units para procesar el valor
                                        try:
                                            unidades = self._extract_units(valor)
                                            if unidades:
                                                valor_dict = {}
                                                for cantidad, unidad in unidades:
                                                    if unidad == "años":
    ...
    def _extraer_datos_antecedentes_familiares(self):
        """
    ...
    def _extraer_datos_inmunohistoquuímica_tumoral(self):
        """
    ...
    def _extraer_datos_clasificacion(self):
        """
    ...
    def _extraer_datos_estadia_tumoral(self):
        """
