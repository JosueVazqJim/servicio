...
def es_ingles(texto, min_palabras=10):
...
def eliminar_frames_duplicados(carpeta, similitud_umbral=5, umbral_texto=0.7):
    """Elimina frames similares dentro de una carpeta considerando también el contenido textual y el idioma inglés."""
    ...
    while i < len(frames):
        ...
        if es_ingles(texto1):
            ...
            print(f"Eliminado {frame1_path} por estar en inglés")
            frames.pop(i)
            continue
...
def procesar_todas_las_carpetas(frame_extraction_folder):
    """Procesa todas las carpetas dentro de 'frame_extraction' eliminando frames duplicados y diapositivas en inglés."""
